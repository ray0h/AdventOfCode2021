...
for seg in data:
  ...
  bounties.append(invalid)
  bounties = list(filter(lambda x: x != None, bounties))

score = reduce(add, [score_errors(e) for e in bounties])
print("Part 1 - Total bounty score: " + str(score))
...
for seg in filtered: 
  ...
  autocomplete = [pairs[open] for open in incompletes]
...
ac_scores.sort()
print("Part 2 - best autocomplete bounty score: " + str(ac_scores[int(len(ac_scores)/2)]))